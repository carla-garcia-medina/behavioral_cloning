...
class BCDataset(torch.utils.data.Dataset):
    ...
    def _generate(self):
        ...
        self.expert_actions = torch.as_tensor(
            np.stack(self.expert_actions)).float()
...
writer = SummaryWriter('runs/bc_cartpole')
...
datasets = {phase: BCDataset(env, num_episodes[phase], ppo1_expert) for phase in [
    'train', 'validation']}

'''
Set hyperparams
'''
pos_weight = (1 - datasets['train'].prop_pos_exmpls) / \
    datasets['train'].prop_pos_exmpls  # use the proportion of pos examples to compute a 'pos_weight' used to normalize binary cross entropy loss used to train the model
# initialize the criterion function, Binary Cross Entropy Loss w/ Sigmoid
criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
# unpack dimension of the observation space from the gym env
...
# initialize the optimizer used for model trianing
...
dataloaders = {'train': torch.utils.data.DataLoader(datasets['train'],  # initialize the train dataloader
               ...
               'validation': torch.utils.data.DataLoader(datasets['validation'],  # initialize the validation dataloader
                                                         # Q: why do I set the batch size to the whole dataset here + omit shuffling?
                                                         batch_size=len(
                   datasets['validation']),
    collate_fn=custom_collate_fn)}
...
for epoch in range(num_epochs+1):
    ...
    for phase in ['train', 'validation']:
        ...
        for batch in iter(dataloaders[phase]):
            ...
            if phase == 'train':
                # Q: why is it important to zero the optimizer gradients before computing the loss?
            ...
            total_incorrect += torch.sum(
                torch.abs((torch.clip(pred_actions, 0, 1) + 0.5).int() - expert_actions))
        # Compute the average accuracy over each phase
        avg_accuracy[phase] = (len(datasets[phase]) -
                               total_incorrect)/len(datasets[phase])
    # Print training/validation stats every 25 epochs
    writer.add_scalar('epoch average train accuracy', avg_accuracy['train'], epoch)
    writer.add_scalar('epoch average validation accuracy', avg_accuracy['validation'], epoch)
    if epoch % 25 == 0:
        print(f'epoch %i | avg train accuracy: %.2f%% | avg valid accuracy: %.2f%%' % (epoch,
                                                                                       np.array(
                                                                                           avg_accuracy['train']).mean() * 100,
...
while True:
    ...
    processed_obs = (torch.as_tensor(
        obs) - datasets['train'].mean_expert_obs)/datasets['train'].std_expert_obs
    with torch.no_grad():
        processed_action = np.clip(
            (model(processed_obs.float()) + 0.5).int().numpy(), -1, 1)[0]
